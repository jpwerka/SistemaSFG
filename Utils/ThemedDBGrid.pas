...
uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, StdCtrls, Grids,
  DBGrids, SfgAlignGrid, DB, ImgList, ThemeSrv, Dialogs;
...
type
  ...
  private
	 ...
	 FOldWndProcEdit: TWndMethod;
	 FOnEditSetFocus: TNotifyEvent;
  ...
  protected
	 ...
	 procedure WndProcEdit(var Message: TMessage);
  ...
  public
	 ...
	 property OnEditSetFocus: TNotifyEvent read FOnEditSetFocus write FOnEditSetFocus;
  end;

  TThemeDBGrid = class(TDBGrid)
  published
	 property AllowTitleClick;
	 property OnEditSetFocus;
  end;

  { TStringAlignGrid }
	TStringAlignGrid = class(SfgAligngrid.TStringAlignGrid) // keep TDBGrid.ClassName = 'TDBGrid'
  private
	 FPaintInfo: TPaintInfo;
	 FCell: TGridCoord; // currently selected cell
	 FAllowTitleClick: Boolean;
  ...
  TThemeStringAlignGrid = class(TStringAlignGrid)
  published
	 property AllowTitleClick;
  end;
  ...
  end;
{
  ...
  TMyDBGridInplaceEdit = class(TInplaceEdit)
  protected
...
constructor TMyDBGridInplaceEdit.Create(Owner: TComponent);
begin
  inherited Create(Owner);
end;

procedure TMyDBGridInplaceEdit.WndProc(var Message: TMessage);
begin
  inherited WndProc(Message);
  case Message.Msg of
	 WM_SETFOCUS:
		begin
		  if Assigned(TThemeDBGrid(Grid).OnEditSetFocus) then
			  TThemeDBGrid(Grid).OnEditSetFocus(Self);
		end;
  end;
end;
}
...
procedure TDBGrid.WndProcEdit(var Message: TMessage);
begin
  FOldWndProcEdit(Message);
  case Message.Msg of
	 WM_SETFOCUS:
		begin
		  if Assigned(OnEditSetFocus) then
			  OnEditSetFocus(InplaceEditor);
		end;
  end;
end;

function TDBGrid.CreateEditor: TInplaceEdit;
  var FInplaceEdit :TInplaceEdit;
begin
  FInplaceEdit := inherited CreateEditor; //TMyDBGridInplaceEdit.Create(Self);
  FOldWndProcEdit := FInplaceEdit.WindowProc;
  FInplaceEdit.WindowProc := WndProcEdit;
  Result := FInplaceEdit;
end;
...
function TStringAlignGrid.BeginColumnDrag(var Origin, Destination: Integer; const MousePt: TPoint): Boolean;
begin
  Result := inherited BeginColumnDrag(Origin, Destination, MousePt);
  FPaintInfo.ColMoving := Result;
end;

function TStringAlignGrid.CentreV(ARect: TRect; ATextHeight: Integer): Integer;
begin
  Result := ((ARect.Bottom - ARect.Top) - ATextHeight) div 2;
end;

function TStringAlignGrid.CentreH(ARect: TRect; ATextWidth: Integer): Integer;
begin
  Result := ((ARect.Right - ARect.Left) - ATextWidth) div 2;
end;

function TStringAlignGrid.ValidCell(ACell: TGridCoord): Boolean;
begin
  Result := (ACell.X <> -1) and (ACell.Y <> -1);
end;

procedure TStringAlignGrid.ColumnMoved(FromIndex, ToIndex: Integer);
begin
  inherited ColumnMoved(FromIndex, ToIndex);
  FPaintInfo.ColMoving := False;
  Invalidate;
...
procedure TStringAlignGrid.CMMouseEnter(var Message: TMessage);
var
  lCell: TGridCoord;
  lPt: TPoint;
begin
  ...
  if (FixedRows > 0) and (lCell.Y = 0) then
  begin
	 InvalidateCell(lCell.X, lCell.Y);
	 // ShowMessage('Passou -> X ' + IntToStr(lCell.X) + ' Y ' + IntToStr(lCell.Y));
  end;
end;

procedure TStringAlignGrid.CMMouseLeave(var Message: TMessage);
begin
  if ValidCell(FCell) then
    InvalidateCell(FCell.X, FCell.Y);
  FCell.X := -1;
...
procedure TStringAlignGrid.WMEraseBkgnd(var Message: TWMEraseBkgnd);
var
  R: TRect;
  Size: TSize;
begin
...
begin
  ...
  begin
    ...
    begin
      FPaintInfo.ColPressedIdx := -1;
		FPaintInfo.ColPressed := False;
		if AllowTitleClick and (goColMoving in Options) then
		  FPaintInfo.MouseInCol := -1;
		lCell := MouseCoord(X,Y);
		if (Button = mbLeft) and (lCell.X >= FixedCols) and (lCell.Y >= 0) and AllowTitleClick and (goColMoving in Options) then
      begin
  ...
  begin
    ...
    begin
      FPaintInfo.MouseInCol := -1;
		...
		if (lCell.X <> FCell.X) or (lCell.Y <> FCell.Y) then
      begin
		  if (lCell.X >= FixedCols) and (lCell.Y >= 0) then
        begin
			 if lCell.Y < FixedRows then
          begin
				lMouseInCol := lCell.X;
            if lMouseInCol <> FPaintInfo.MouseInCol then
            begin
...
begin
  ...
  begin
	 // Reset the inherited options but remove the goFixedVertLine and goFixedHorzLine values
	 // as that causes the titles and indicator panels to have a black border
	 Self.Options := Self.Options - [goFixedVertLine];
	 Self.Options := Self.Options - [goFixedHorzLine];
  end;
  inherited Paint;
end;

procedure TStringAlignGrid.DrawCell(ACol, ARow: Integer; ARect: TRect; AState: TGridDrawState);
var
  Details: TThemedElementDetails;
  lCaptionRect: TRect;
  lCellRect: TRect;
...
begin
   lCellRect := ARect;
   if (ARow = 0) and (ACol - FixedCols >= 0) and (FixedRows > 0) and ThemeServices.ThemesEnabled then
   begin
      lCaptionRect := ARect;
      if not FPaintInfo.ColPressed or (FPaintInfo.ColPressedIdx <> ACol) then
      begin
		...
		else if AllowTitleClick and (goColMoving in Options) then
		begin
         Details := ThemeServices.GetElementDetails(thHeaderItemPressed);
			InflateRect(lCaptionRect, -1, 1);
      end
      ...
      begin
		...
		//ShowMessage(' Left ' + IntToStr( lCellRect.left) +  ' Right ' + IntToStr( lCellRect.right) +' Top ' + IntToStr( lCellRect.top) +' Bottom ' + IntToStr( lCellRect.bottom));
		ThemeServices.DrawElement(Canvas.Handle, Details, lCellRect);
		Canvas.Brush.Style := bsClear;
      Canvas.Font.Assign(GetFontCell(ACol, ARow));
      lStr := Cells[ACol,ARow];
      TextY := lCaptionRect.Top + CentreV(lCaptionRect, Canvas.TextHeight(lStr));
      case GetAlignCell(ACol, ARow) of
   	   SfgAligngrid.alRight:
            TextX := lCaptionRect.Right - 2 - Canvas.TextWidth(lStr);
   	   SfgAligngrid.alCenter:
            TextX := lCaptionRect.Left + CentreH(lCaptionRect, Canvas.TextWidth(lStr));
         else
            TextX := lCaptionRect.Left + 2;
      end;
	...
	else if (ACol < FixedCols) and (FixedCols > 0) and ThemeServices.ThemesEnabled then
   begin
      // indicator column
...
var
  ...
  OrgTStringAlginGrid_NewInstance: Pointer;
...
function TStringAlginGrid_NewInstance(AClass: TClass): TObject;
begin
  Result := TStringAlignGrid.NewInstance;
end;
...
initialization
  ...
  OrgTStringAlginGrid_NewInstance := GetVirtualMethod(TStringAlignGrid, vmtNewInstance);
  SetVirtualMethod(SfgAligngrid.TStringAlignGrid, vmtNewInstance, @TStringAlginGrid_NewInstance);

finalization
  SetVirtualMethod(DBGrids.TDBGrid, vmtNewInstance, OrgTDBGrid_NewInstance);

  SetVirtualMethod(SfgAligngrid.TStringAlignGrid, vmtNewInstance, OrgTStringAlginGrid_NewInstance);
