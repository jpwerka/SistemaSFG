...
BEGIN
   ...
   DECLARE cq_index CURSOR FOR
   SELECT idx.table_name, idx.column_name 
   ...
   INSERT INTO tt_meta_fields
      (table_name, column_name, column_order,
       ...
       column_checkvalues, column_editcmp, column_format)
   SELECT col.table_name, col.COLUMN_NAME, col.ordinal_position column_order,
          ...
          CASE WHEN col.numeric_scale IS NULL THEN 0 ELSE col.numeric_scale END column_precision,
   ...
   OPEN cq_index;
   read_loop: LOOP
       FETCH cq_index INTO l_table_name,
                           l_column_name;
      IF not_found THEN
         LEAVE read_loop;
      END IF;
      ...
      UPDATE tt_meta_fields
       ...
       WHERE table_name = l_table_name 
         ...
         AND (fk_table_name != l_fktable_name 
           OR fk_column_name != l_fkcolumn_name);
