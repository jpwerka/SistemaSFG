...
//---------------------------------------------------------------------------
void __fastcall THackfrmBase02::BtParametroClick(TObject *Sender)
{
...
void CreateButtonParametro(TForm *pForm)
{
   ...
   btParametro->Hint = "Indicar a forma de pagamento para ser|utilizada na solicitação da senha do cliente.";
	...
	: TForm(Owner,1)
{
   ...
   pEdtPgto->Text = VarToStr(ValidacoesDB::GetParamValue(AppConnection,"CALL get_parametro_numero('forma_pagto_senha');"));
}
//---------------------------------------------------------------------------
void __fastcall TCVdp1010A::BtOKClick(TObject *Sender)
{
   ...
   if (pEdtPgto->Text.Length() == 0) {
      MessageBox(NULL,"Informe o código da forma de pagamento para solicitar a senha.","Forma pagamento",APP_ATENTION);
      return;
   }
   sSQL = "CALL set_parametro_numero('forma_pagto_senha'," + pEdtPgto->Text + ",'" + AppUser->Usuario + "');";
